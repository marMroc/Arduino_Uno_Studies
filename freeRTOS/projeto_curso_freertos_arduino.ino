...
void setup() 
{
    ...
    /* Criação das tarefas */
    xTaskCreate(
      task_lcd                     /* Funcao a qual esta implementado o que a tarefa deve fazer */
...
void loop() 
{
    /* Tudo é feito pelas tarefas. Logo, nada precisa / deve ser 
       programado aqui */
}
...
/* task_lcd: tarefa responsável por escrever as leituras dos sensores no
             display LCD */
void task_lcd( void *pvParameters )
{
    float distancia = 0.0;
...
/* task_sensor_MQ2: tarefa responsável por ler o sensor MQ-2 e enviar, 
                       através de uma fila, cada leitura para a tarefa que 
...
void task_sensor_MQ2( void *pvParameters )
{
    ...
    while(1)
    {
       ...
       /* Escreve leitura na serial. Aqui, a tentativa de se tomar o controle
          do semáforo é feita até o tempo definido em TEMPO_PARA_AGUARDAR_SEMAFORO.
          Caso não for possível tomar o controle do semáforo até este tempo (pois outra
          tarefa o está utilizando), nada é escrito na Serial
       */
       if ( xSemaphoreTake(semaforo_serial, TEMPO_PARA_AGUARDAR_SEMAFORO ) == pdTRUE )
       {
        ...
        /* Aguarda tempo definido em MQ2_TEMPO_ENTRE_LEITURAS 
           para fazer próxima leitura */
...
void task_sensor_ultrassonico( void *pvParameters )
{
    ...
    while(1)
    {
       ...
       */
       ...
       /* Aguarda tempo definido em ULTRASSONICO_TEMPO_ENTRE_LEITURAS 
          para fazer próxima leitura */
